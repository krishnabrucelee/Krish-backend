			...
			for (int i = 0, size = capacityArrayJSON.length(); i < size; i++) {
				...
				case "0":
					...
					if(memory < Long.valueOf(vm.getComputeOffering().getMemory().toString())){
						if (vm.getProjectId() != null) {
							errMessage = "Maximum number of resources of type 'memory' for project "+vm.getProject().getName() +" has been exceeded.";
						}
						else{
							errMessage = "Maximum number of resources of type 'memory' for current domain "+ domainRepository.findOne(vm.getDepartment().getDomainId()).getName() +" has been exceeded.";
						}
					}
					break;
				case "1":
				...
				case "2":
					...
					if(secondaryStorage < tempCount){
						if (vm.getProjectId() != null) {
							errMessage = "Maximum number of resources of type 'seconday storage' for project "+vm.getProject().getName() +" has been exceeded.";
						}
						else{
							errMessage = "Maximum number of resources of type 'seconday storage' for current domain "+ domainRepository.findOne(vm.getDepartment().getDomainId()).getName() +" has been exceeded.";
						}
					}
					break;
				case "3":
					...
					if(primaryStorage < vm.getTemplate().getSize()){
						if (vm.getProjectId() != null) {
							errMessage = "Maximum number of resources of type 'primary storage' for project "+vm.getProject().getName() +" has been exceeded.";
						}
						else{
							errMessage = "Maximum number of resources of type 'primary storage' for current domain "+ domainRepository.findOne(vm.getDepartment().getDomainId()).getName() +" has been exceeded.";
						}
					}
					break;
				case "4":
