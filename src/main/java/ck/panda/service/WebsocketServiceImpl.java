        ...
        if (event != null) {
            if (event.getEvent() != null) {
				if (event.getEventType().equals(Event.EventType.ASYNC)) {
					String instanceResponse = cloudStackInstanceService.queryAsyncJobResult(event.getJobId(),
				...
				if (event.getEventType().equals(Event.EventType.RESOURCESTATE)) {
					if (eventObject.has(EventTypes.RESOURCE_STATE) && eventObject.has(EventTypes.OLD_RESOURCE_STATE)) {
						if (!eventObject.getString(EventTypes.RESOURCE_STATE)
								.equalsIgnoreCase(eventObject.getString(EventTypes.OLD_RESOURCE_STATE))) {
									...
									if (volume != null) {
										if (event.getMessage().equalsIgnoreCase("Expunged")) {
											...
											messagingTemplate.convertAndSend(
													CloudStackConstants.CS_RESOURCE_MAP + event.getEvent(), message);
										}
										if (event.getMessage().equalsIgnoreCase("Uploaded")) {
											...
											String response = csVolumeService.listVolumes(CloudStackConstants.JSON,
													volumeMap);
											...
											if (responseObject.has(CS_VOLUME)) {
												...
												volume.setStatus(
														Volume.Status.valueOf(event.getMessage().toUpperCase()));
												volumeService.update(volume);
												messagingTemplate.convertAndSend(
								...
								if (event.getEvent().equals("VirtualMachine")) {
									...
									if (vmInstance != null) {
										...
										if (resourceCurrentStatus != null
											...
											if (event.getMessage().equals(EventTypes.EVENT_STATUS_STOPPED)
													|| event.getMessage().equals(EventTypes.EVENT_STATUS_DESTROYED)) {
											...
											if (eventObject.getString(EventTypes.OLD_RESOURCE_STATE)
												...
												if (vmInstance.getProjectId() != null) {
													...
													updateResourceCountService.QuotaUpdateByResourceObject(vmInstance,
															"RestoreInstance", vmInstance.getDepartmentId(),
															"Department", "Update");
												}
											}
											if (eventObject.getString(EventTypes.OLD_RESOURCE_STATE)
												...
												if (vmInstance.getProjectId() != null) {
													updateResourceCountService.QuotaUpdateByResourceObject(vmInstance,
															"Destroy", vmInstance.getProjectId(), "Project",
															"delete");
												} else {
													updateResourceCountService.QuotaUpdateByResourceObject(vmInstance,
															"Destroy", vmInstance.getDepartmentId(),
															"Department", "delete");
												}
											}
											if (event.getMessage().equalsIgnoreCase("Expunging")) {
												if (vmInstance.getProjectId() != null) {
													updateResourceCountService.QuotaUpdateByResourceObject(vmInstance,
															"Expunging", vmInstance.getProjectId(), "Project",
															"delete");
												} else {
													updateResourceCountService.QuotaUpdateByResourceObject(vmInstance,
															"Expunging", vmInstance.getDepartmentId(),
															"Department", "delete");
												}
											}
														...
														for (int i = 0, size = vmListJSON.length(); i < size; i++) {
															...
															if (vmInstance.getHostId() != null) {
																vmInstance.setPodId(convertEntityService
																		.getPodIdByHost(convertEntityService.getHostId(
																				CsVmInstance.getTransHostId())));
															}
															// 3. Update vm for
															// user vm
															// creation.
